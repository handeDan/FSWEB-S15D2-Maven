...
public class StringSet {
    public static Set<String> findUniqueWords(){
        ...
        //tüm noktalama işaretlerini ve özel karakterleri kaldırıyoruz: ? javada özel bir karakter olduğu için tek başına yazsaydık ? => text.replaceAll("\\?", "") olarak yazılmalıydı.(\\ : escape character)
        text = text.replaceAll("[^a-zA-Z\\s]", "").toLowerCase().trim();

        //kelimeleri boşluklara göre ayırıp bir string arrayine topluyoruz:
        // "\\" : escape character.
        // "s": boşluk (' '), tab ('\t'), yeni satır ('\n'), vb. tüm boşluk karakterlerini kapsar.
        // "+": bir veya daha fazla tekrar anlamına gelir.
        // "\\s+" : bir veya daha fazla boşluk karakteri
        String[] words = text.split("\\s+");

        //TreeSet ile kelimeleri alfabetik sıralıyoruz:
        Set<String> uniqueWords = new TreeSet<>();

        //her bir kelimeyi loop'layıp unique olanları TreeSet'imize ekliyoruz:
